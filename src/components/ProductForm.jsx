...
const ProductForm = ({ product, initialValues }) => {
const navigate = useNavigate();
const activeProduct = product || initialValues;
      ...
      <div className="container px-3 px-md-4">
        <div className="card shadow-sm border-0 rounded-4 bg-white overflow-hidden">
          <div className="text-white p-4 text-center" style={{ background: "linear-gradient(45deg, #198754, #20c997)" }} >
            <h3 className="fw-bold mb-0"> {activeProduct ? "Edit Product Details" : "Add New Medicine"} </h3>
            </div>
          <div className="card-body p-4 p-md-5">
            <form onSubmit={handleSubmit(submit)} className="row g-4">
              ...
              <div className="col-12 mt-4">
                <button type="submit"  disabled={isSubmitting} className="btn btn-success text-white w-100 py-2 fw-bold rounded-3" > {isSubmitting ? "Saving..." : activeProduct ? "Update Product" : "Add Product"}</button>
              </div>
            </form>
          </div>
        </div>
